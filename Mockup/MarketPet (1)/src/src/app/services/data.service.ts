...
import { Firestore, collection, doc, addDoc, getDocs, getDoc, updateDoc, deleteDoc, query, where, orderBy, limit } from '@angular/fire/firestore';
import { Observable, from, map } from 'rxjs';
import { Pet, Provider, Appointment, ServiceCategory } from '../models/user.model';
...
export class DataService {
  ...
  async createProvider(providerData: Omit<Provider, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    ...
    const docRef = await addDoc(providersCollection, newProvider);
    return docRef.id;
  }
