...
class PaginaPremium extends StatelessWidget {
  ...
  Future<void> _actualizarEstado(bool activar) async {
    await FirebaseFirestore.instance.collection('users').doc(idUsuario).set({
      'esPremium': activar,
      ...
      builder: (context, snapshot) {
        ...
        return ListView(
          ...
          children: [
            ListTile(
              leading: Icon(
                esPremium ? Icons.workspace_premium : Icons.workspace_premium_outlined,
                color: Theme.of(context).colorScheme.primary,
              ),
              title: Text(esPremium ? 'Eres cliente Premium' : 'Plan gratuito'),
              subtitle: Text(esPremium
